...
import org.bungee.rope.bungy.exception.WrapMessageException;
...
public interface IAssert {
	 ...
	 * {{{@link #newException(Throwable)}}}, 作为最后创建的异常的 cause 属性.
	 *
	...
	/**
	 * 创建异常. 先使用 {@code errMsg} 和 {@code t} 创建一个 {@link WrapMessageException} 异常,
	 * 再以入参的形式传给 {{{@link #newException(Throwable)}}}, 作为最后创建的异常的 cause 属性.
	 *
	 * @param errMsg 自定义的错误信息
	 * @param args   多参数
	 */
	 ...
	 *
	 * @param obj    待判断对象
	 * @param errMsg 自定义的错误信息. 支持 {index} 形式的占位符, 比如: errMsg-用户[{0}]不存在, args-1001,
	 *               最后打印-用户[1001]不存在
	 ...
	 *
	 * @param expression 待判断布尔变量
	 * @param errMsg     自定义的错误信息. 支持 {index} 形式的占位符, 比如: errMsg-用户[{0}]不存在,
	 *                   args-1001, 最后打印-用户[1001]不存在
	 * @param args       message占位符对应的参数列表
	 */
